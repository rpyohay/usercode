...
\begin{figure}
	...
	\caption{$\sigma$ of a Gaussian fit to the x- and y-components of calibrated \MET vs. the calibrated PF $E_{T}$ sum in a sample of events containing at least two jets with $p_{T} > 25$ GeV.  $\sigma$ is calibrated such that the \MET scale is equal for all three algorithms.  PF $\sum E_{T}$ is corrected, on average, to the particle level using a Pythia v8 simulation \cite{Pythia}.  The blue markers (data) and line (MC) refer to PF jets.  Reprinted from Fig. 13 of ref. \cite{CMS_MET_paper}.}
...
%show MC plots comparing N-1 distributions of ID variables for photons (reco::Photon matched to gen photon) and electrons (reco::Photon matched to gen electron)
	%need to ntuple the Fall11 MC
%same for data, but distinguish electron and photon by the presence/absence of a pixel seed
%same for data, but compare tag electrons to candidate photons
...
%H/E, R9, sigmaIetaIeta, combined isolation, individual isolations
...
Using a tag and probe method with $Z\rightarrow ee$ data events, the efficiency for photons is calculated as
...
%\hline
%\hline
%$I_{\mathrm{ECAL}}$ & $< 0.012E_{T} + 6$ GeV & ($I_{\mathrm{ECAL}}$ \&\& \\
%$I_{\mathrm{HCAL}}$ & $< 0.005E_{T} + 4$ GeV & $I_{\mathrm{HCAL}}$ \&\& \\
%$I_{\mathrm{track}}$ & $< 0.002E_{T} + 4$ GeV & $I_{\mathrm{track}}$) $||$ \\
%$R9$ & $> 0.8$ & $R9$ \\
%\hline
%$H/E$ & $< 0.05$ & \\
%\hline
...
%
%The tag and probe criteria are shown in Table~\ref{tab:tag_probe_cuts}.
%
%\begin{table}[hcbp]
...
%\end{table}
%
%Two samples of tag-probe pairs are formed: one in which the probe passes the photon ID selection in Table~\ref{tab:photon_ID_cuts}, and one in which the probe fails the selection.  The tag-probe invariant mass distributions for each sample are fitted simultaneously using an extended likelihood fit to extract the efficiency $\epsilon_{e}^{\mathrm{data}}$.  The fitting tool used is RooFit, as implemented in the CMSSW package PhysicsTools/TagAndProbe.  For this study, CMSSWv4.2.8 was used.  For both samples, the signal shape is assumed to be a Crystal Ball convoluted the Z generated lineshape, while the background shape is a PDF that describes the falling background as well as the kinematic turn-on at low invariant mass.  The background PDF, called ``RooCMSShape", is given by
%
%\begin{eqnarray}
...
%
%\begin{table}[hcbp]
%\caption{Parameter values for the signal and background PDFs for the different samples.  The background PDF applies to all efficiency bins except the four lowest $E_{T}$ bins, which use a floating RooCMSShape background).}
%\centering
%\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
%\hline
%& \multicolumn{4}{c|}{Crystal Ball fit parameters} & \multicolumn{4}{c|}{\begin{tabular}[c]{@{}c@{}}RooCMSShape\\fit parameters\end{tabular}} \\
%\hline
%PDF & $\mu$ & $\sigma$ & $\alpha$ & n & $\mu$ & $\alpha$ & $\beta$ & $\gamma$ \\
%\hline
%\begin{tabular}[c]{@{}c@{}}Tag-pass\\signal\end{tabular} & $\left[-1.0, 1.0\right]$ & $\left[1.0, 3.0\right]$ & 0.87 & 97.0 & N/A & N/A & N/A & N/A \\
%\hline
%\begin{tabular}[c]{@{}c@{}}Tag-fail\\signal\end{tabular} & $\left[-1.0, 1.0\right]$ & $\left[1.0, 3.0\right]$ & 0.73 & 134.9 & N/A & N/A & N/A & N/A \\
%\hline
%\begin{tabular}[c]{@{}c@{}}Tag-pass\\background\end{tabular} & N/A & N/A & N/A & N/A & 65.0 & 61.949 & 0.04750 & 0.01908 \\
%\hline
%\begin{tabular}[c]{@{}c@{}}Tag-fail\\background\end{tabular} & N/A & N/A & N/A & N/A & $\alpha$ & [50.0, 100.0] & 0.065 & 0.048 \\
%\hline
%\end{tabular}
%\label{tab:PDFs}
%\end{table}

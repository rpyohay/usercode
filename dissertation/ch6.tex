...
\begin{figure}
	\centering
	\subfloat[EB.]{\label{fig:Z_mass_vs_corrections_EB}\includegraphics[scale=1.2]{Z_mass_vs_corrections_EB}}
	\\
	\subfloat[EE.]{\label{fig:Z_mass_vs_corrections_EE}\includegraphics[scale=1.2]{Z_mass_vs_corrections_EE}}
...
\begin{figure}
	\centering
	\subfloat[Events with $\geq0$ jets.]{\label{fig:f_comb_iso_upper_bound_optimization_geq0j}\includegraphics[scale=1.2]{f_comb_iso_upper_bound_optimization_geq0j}}
	\\
	\subfloat[Events with $\geq1$ jet.]{\label{fig:f_comb_iso_upper_bound_optimization_geq1j}\includegraphics[scale=1.2]{f_comb_iso_upper_bound_optimization_geq1j}}
...
Has pixel seed & Yes & --- \\
\hline
Track match type & General track\footnote{A general track is reconstructed with the CMS standard combinatorial track finder \cite{Adam:934067}.} & --- \\
\hline
...
\begin{figure}
	\centering
	\subfloat[Data.]{\label{fig:low_ET_bin_data}\includegraphics[scale=0.5]{low_ET_bin_data}}
	\\
	\subfloat[MC.  The purple points are the background MC (photon + jet, $W$, QCD, and $t\bar{t}$).]{\label{fig:low_ET_bin_MC}\includegraphics[scale=0.5]{low_ET_bin_MC}}
...
\begin{figure}
	\centering
	\subfloat[Data.]{\label{fig:mid-eta_bin_data}\includegraphics[scale=0.5]{mid-eta_bin_data}}
	\\
	\subfloat[MC.  The purple points are the background MC (photon + jet, $W$, QCD, and $t\bar{t}$).]{\label{fig:mid-eta_bin_MC}\includegraphics[scale=0.5]{mid-eta_bin_MC}}
...
The effect of pileup is studied by comparing the efficiencies $\epsilon_{e}^{\mathrm{data}}$ and $\epsilon_{e}^{\mathrm{MC}}$ vs. the number of primary vertices ($N_{\mathrm{PV}}$) in the event.  The efficiency only drops a few percent for events with large $N_{\mathrm{PV}}$ after using pileup-corrected isolation cuts, as can be seen in Figure~\subref*{fig:eff_vs_NPV}.  The MC tracks the data, and the scale factor is flat vs. $N_{\mathrm{PV}}$, as shown in Fig.~\subref*{fig:scale_factor_vs_NPV}.

\begin{figure}
	\centering
	\subfloat[Electron efficiency vs. $N_{\mathrm{PV}}$.  Blue circles are data, green squares are MC.]{\label{fig:eff_vs_NPV}\includegraphics[scale=0.25]{eff_vs_NPV}}
	\hspace{1cm}

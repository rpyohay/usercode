\documentclass[dissertation_bw.tex]{subfiles} 
\begin{document}
...
\begin{figure}
	\centering
	\subfloat[Dijet production via $gg$ and $qg$ interactions.]{\label{fig:dijet}\includegraphics[scale=1.0]{dijet_cropped}}
	\\
	\subfloat[Photon + jet production via $qg$ interactions.]{\label{fig:qg_to_qa}\includegraphics[scale=1.0]{qg_to_qa}}
	\hspace{1cm}
	\subfloat[Diphoton production via $q\bar{q}$ and $gg$ interactions.]{\label{fig:diphoton}\includegraphics[scale=1.0]{diphoton_cropped}}
%	\hspace{1cm}
%	\subfloat[$Z\gamma$ production.]{\label{fig:Zgamma}\includegraphics[scale=1.0]{Zgamma_cropped}}
...
\begin{figure}
	\centering
	\subfloat[$W\gamma$ production.]{\label{fig:Wgamma}\includegraphics[scale=1.0]{qq_to_W_to_enu_to_enua}}
	\hspace{1cm}
	\subfloat[$W$ + jet production.]{\label{fig:W_jet}\includegraphics[scale=1.0]{qg_to_qW_to_qenu}}
	\\
	\subfloat[$t\bar{t}$ production.]{\label{fig:ttbar}\includegraphics[scale=1.0]{gg_to_tt_to_WbWb_to_ffbenub_to_ffbenuba}}
...
\begin{figure}
	\centering
	\subfloat[$\gamma\gamma$.]{\label{fig:gg_MET_vs_di-EM_pT}\includegraphics[scale=0.3]{gg_MET_vs_di-EM_pT_bw}}
	\\
	\subfloat[$ee$.]{\label{fig:ee_MET_vs_di-EM_pT}\includegraphics[scale=0.3]{ee_MET_vs_di-EM_pT_bw}}
	\\
	\subfloat[$\mathit{ff}$.]{\label{fig:ff_MET_vs_di-EM_pT}\includegraphics[scale=0.3]{ff_MET_vs_di-EM_pT_bw}}
...
\begin{figure}
	\centering
	\subfloat[Leading electron in $ee$ events.]{\label{fig:4684pb-1_eLeading_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_eLeading_ETBias_log_bw}}
	\hspace{1cm}
	\subfloat[Trailing electron in $ee$ events.]{\label{fig:4684pb-1_eTrailing_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_eTrailing_ETBias_log_bw}}
	\\
	\subfloat[Leading fake in $\mathit{ff}$ events.]{\label{fig:4684pb-1_fLeading_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_fLeading_ETBias_log_bw}}
	\hspace{1cm}
	\subfloat[Trailing fake in $\mathit{ff}$ events.]{\label{fig:4684pb-1_fTrailing_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_fTrailing_ETBias_log_bw}}
	\\
	\subfloat[Leading photon in $\gamma\gamma$ events.]{\label{fig:4684pb-1_gLeading_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_gLeading_ETBias_log_bw}}
	\hspace{1cm}
	\subfloat[Trailing photon in $\gamma\gamma$ events.]{\label{fig:4684pb-1_gTrailing_ETBias_log}\includegraphics[scale=0.3]{4684pb-1_gTrailing_ETBias_log_bw}}
...
\begin{figure}
	\centering
	\subfloat[0 jets.]{\label{fig:0_jet_dijet_pT}\includegraphics[scale=0.3]{0_jet_dijet_pT}}
	\\
	\subfloat[1 jet.]{\label{fig:1_jet_dijet_pT}\includegraphics[scale=0.3]{1_jet_dijet_pT}}
	\\
	\subfloat[$\geq$ 2 jets.]{\label{fig:2_jet_dijet_pT}\includegraphics[scale=0.3]{2_jet_dijet_pT}}
...
Fits to the $ee$ and $e\gamma$ invariant mass spectra are shown in Figure~\ref{fig:feg_fits}.  Figure~\subref*{fig:feg_vs_pT} indicates that there is some dependence of $f_{e\rightarrow\gamma}$ on the electron $p_{T}$, but since the the bulk of the electrons have $p_{T}\sim45$ GeV, applying a $p_{T}$-dependent mis-identification rate has a negligible effect on the final background estimate.  (Note that all fit parameters are floating in the $p_{T}$-dependent fits.)  The dependence on $\eta$ is small, as shown in Figure~\subref*{fig:feg_vs_eta}.  Therefore, a constant misidentification rate (derived from all $ee$ and $e\gamma$ events), rather than a $p_{T}$- and $\eta$-dependent misidentification rate, is used in the final electroweak background estimate, with the difference between the constant rate and the rate for electrons with $p_{T}$ between 25 and 40 GeV (the range in which the bulk of the trailing photons in the $\gamma\gamma$ sample lie) taken as a systematic error.
...
\begin{figure}
	\centering
	\subfloat[$f_{e\rightarrow\gamma}$ vs. electron $p_{T}$.  For the lowest $p_{T}$ bin, the fit to the $e\gamma$ spectrum does not converge well, so the $Z$ signal fraction is fixed to the value in Fig.~\subref*{fig:meg_fit}.]{\label{fig:feg_vs_pT}\includegraphics[scale=0.3]{feg_vs_pT}}
	\hspace{1cm}
...
\begin{figure}
	\centering
	\subfloat[Ratio of $\gamma\gamma$ to $ee$ \MET distributions.]{\label{fig:gg_over_ee_Type-I_MET_corrections_vs_uncorrected_MET_zoom}\includegraphics[scale=0.4]{gg_over_ee_Type-I_MET_corrections_vs_uncorrected_MET_zoom}}
	\\
	\subfloat[Ratio of $\gamma\gamma$ to $\mathit{ff}$ \MET distributions.]{\label{fig:gg_over_ff_Type-I_MET_corrections_vs_uncorrected_MET_zoom}\includegraphics[scale=0.4]{gg_over_ff_Type-I_MET_corrections_vs_uncorrected_MET_zoom}}
	\\
	\subfloat[Ratio of $ee$ to $\mathit{ff}$ \MET distributions.]{\label{fig:ee_over_ff_Type-I_MET_corrections_vs_uncorrected_MET_zoom}\includegraphics[scale=0.4]{ee_over_ff_Type-I_MET_corrections_vs_uncorrected_MET_zoom}}
...
%The dijet \pT reweighting method utilizes jets corrected for imperfect calorimeter response (see Sec.~\ref{sec:Jets and Missing Transverse Energy} for a description of the jet reconstruction and correction procedure).  Since the applied jet energy scale (JES) factor has an error associated to it due to the limitations of the JES derivation (\cite{1748-0221-6-11-P11002} and Sec.~\ref{sec:Jets and Missing Transverse Energy}), this uncertainty must be propagated to the uncertainty on the dijet \pT weights.
%
...
\begin{figure}
	\centering
	\subfloat[$ee$ + $e\gamma$ and $\mathit{ff}$ + $e\gamma$.  The widths of the bands correspond to the errors given in Table~\ref{tab:total_background_prediction_errors}, excluding the error associated with the difference between the $ee$ and $\mathit{ff}$ QCD estimates for the $\mathit{ff}$ + $e\gamma$ \MET distribution.]{\label{fig:MET_final_ee_ff_eg}\includegraphics[scale=0.35]{MET_final_ee_ff_egbw}}
	\hspace{1cm}
	\subfloat[$\mathit{ff}$ + $e\gamma$.  The widths of the bands correspond to the errors given in Table~\ref{tab:total_background_prediction_errors}, including the error associated with the difference between the $ee$ and $\mathit{ff}$ QCD estimates.]{\label{fig:MET_final_ff_eg}\includegraphics[scale=0.35]{MET_final_ff_egbw}}
...
\begin{figure}
	\centering
	\subfloat[$ee$ + $e\gamma$ and $\mathit{ff}$ + $e\gamma$.  The widths of the bands correspond to the errors given in Table~\ref{tab:total_background_prediction_errors}, excluding the error associated with the difference between the $ee$ and $\mathit{ff}$ QCD estimates for the $\mathit{ff}$ + $e\gamma$ \MET distribution.]{\label{fig:MET_final_ee_ff_eg_geq1j}\includegraphics[scale=0.35]{MET_final_ee_ff_eg_geq1bw}}
	\hspace{1cm}
	\subfloat[$\mathit{ff}$ + $e\gamma$.  The widths of the bands correspond to the errors given in Table~\ref{tab:total_background_prediction_errors}, including the error associated with the difference between the $ee$ and $\mathit{ff}$ QCD estimates.]{\label{fig:MET_final_ff_eg_geq1j}\includegraphics[scale=0.35]{MET_final_ff_eg_geq1bw}}
	\caption{\MET distribution of the $\gamma\gamma$ + $\geq$1 jet search sample (black circles) along with the predicted \MET distributions of the QCD and electroweak backgrounds (small circle band for $ee$ QCD prediction + electroweak prediction, slanted dashed band for $\mathit{ff}$ QCD prediction + electroweak prediction).  The electroweak background prediction is shown in the crosshatched band.  The bottom plots show the ratio of the $\gamma\gamma$ \MET distribution to the $ee$ + $e\gamma$ background distribution (open squares) and $\mathit{ff}$ + $e\gamma$ background distribution (filled squares).}
	\label{fig:MET_final_geq1j}
\end{figure}

\begin{table}[hcbp]
\caption{Observed numbers of two-photon events, $ee$ and $\mathit{ff}$ background estimates and their errors, and expected numbers of two-photon events from two representative GGM SUSY models (details of MC simulation given in Chapter~\ref{chap:Interpretation of Results in Terms of GMSB Models}) for the \MET bins used in the search.  The $ee$ estimate is shown for comparison; the $\mathit{ff}$ estimate is the official result used in the interpretation.  Errors on the background estimates are detailed in Tables~\ref{tab:ee_background_prediction_errors},~\ref{tab:ff_background_prediction_errors},~\ref{tab:eg_background_prediction_errors}, and~\ref{tab:total_background_prediction_errors}.  Errors on the expected numbers of GGM events are purely statistical.}
\centering
...
\hline
\begin{tabular}{@{}c@{}}Predicted\\background\\($\mathit{ee}$ + $e\gamma$)\\\end{tabular} & 317 $\pm$ 14.1 & 90.2 $\pm$ 7.77 & 39.6 $\pm$ 5.75 & 25.1 $\pm$ 5.66 & 28.9 $\pm$ 6.70 \\
\hline
...
\begin{table}[hcbp]
\caption{Observed numbers of two-photon + $\geq$1-jet events, $ee$ and $\mathit{ff}$ background estimates and their errors, and expected numbers of two-photon + $\geq$1-jet events from two representative GGM SUSY models (details of MC simulation given in Chapter~\ref{chap:Interpretation of Results in Terms of GMSB Models}) for the \MET bins used in the search.  The $ee$ estimate is shown for comparison; the $\mathit{ff}$ estimate is the official result used in the interpretation.  Errors on the background estimates are detailed in Tables~\ref{tab:ee_background_prediction_errors},~\ref{tab:ff_background_prediction_errors},~\ref{tab:eg_background_prediction_errors}, and~\ref{tab:total_background_prediction_errors}.  Errors on the expected numbers of GGM events are purely statistical.}
\centering
{\footnotesize\begin{tabular}{|c|c|c|c|c|c|}
\hline
\multicolumn{1}{|c|}{\multirow{2}{*}{Source}} & \multicolumn{5}{c|}{No. events} \\
...
\hline
\begin{tabular}{@{}c@{}}Predicted\\background\\($\mathit{ee}$ + $e\gamma$)\\\end{tabular} & 171 $\pm$ 10.2 & 52.7 $\pm$ 5.74 & 25.1 $\pm$ 4.76 & 10.2 $\pm$ 5.04 & 13.1 $\pm$ 5.74 \\
\hline
